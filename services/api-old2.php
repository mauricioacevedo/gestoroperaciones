	...
	class API extends REST {
		...
		*/
		...
		private function csvHistoricos(){
                        ...
                        $query=" SELECT ".
                        " pedido_id,subpedido_id,solicitud_id,municipio_id, fuente, actividad, fecha_fin, estado,duracion,accion,concepto_final,concepto_anterior,user,idllamada,motivo,nuevopedido ".
                        " from pedidos where ".
			" fecha_fin between '$fechaIni 00:00:00' and '$fechaFin 23:59:59' ";
                        ...
                        if($r->num_rows > 0){
                                ...
                                fputcsv($fp, array('pedido_id','subpedido_id','solicitud_id','municipio_id',' fuente',' actividad',' fecha_fin',' estado', 'duracion','accion','concepto_final','concepto_anterior','user','idllamada','motivo','nuevopedido'));
		...
		private function csvFenixNal(){
		 ...
		 private function insertPedidoReconfiguracion(){
                       if($this->get_request_method() != "POST"){
                        ...
                        $pedido = json_decode(file_get_contents("php://input"),true);
                        $column_names = array('pedido', 'fuente', 'actividad','estado', 'user','duracion','fecha_inicio','fecha_fin','PEDIDO_ID','SUBPEDIDO_ID','SOLICITUD_ID','MUNICIPIO_ID','CONCEPTO_ANTERIOR','motivo','idllamada','nuevopedido');
                        $keys = array_keys($pedido);
                        $columns = '';
                        $values = '';
                        $fecha_estado='';
					...
					$concepto_final=$CONCEPT;
                                ...
                                if($estadum=="MALO"){
                                        ...
                                        $r = $this->mysqli->query($query) or die($this->mysqli->error.__LINE__);
                                        $sqlupdate="update informe_petec_pendientesm set FECHA_FINAL='$today',STATUS='$estadum' WHERE ID=$iddd ";
		...
		private function insertPedido(){
                       if($this->get_request_method() != "POST"){
                        ...
                        $pedido = json_decode(file_get_contents("php://input"),true);
                        $column_names = array('pedido', 'fuente', 'actividad','estado','motivo', 'user','duracion','fecha_inicio','fecha_fin','PEDIDO_ID','SUBPEDIDO_ID','SOLICITUD_ID','MUNICIPIO_ID','CONCEPTO_ANTERIOR');
                        $keys = array_keys($pedido);
                        $columns = '';
                        $values = '';
			$fecha_estado='';
		...
		private function pendientesPorPlaza(){
			...
			" group by C1.PLAZA order by count(*) DESC";
                        ...
                        if($r->num_rows > 0){
                                ...
                                $this->response($this->json(array($result,$resultBogota)), 200); // send user details
                        }
		...
		//este demepedido valida contra fenix antes de suministrar el pedido...
			...
			if($concepto=="PETEC"){
				...
				$concepto=" and b.CONCEPTO_ID in ('PETEC','15') and (b.TIPO_ELEMENTO_ID IN ('E2MB','P2MB','INSTIP','CNTXIP','SEDECX','PLANT','PLP','PTLAN','MTLAN', 'PMULT','EPCM','PPCM','PBRI','PPRI','INSTIP','TV','TP','PBRI','BDID','TDID','BDIDE1','TDIDE1','BDODE1','TDODE1','SLL','TC','SLLBRI','TCBRI','SLLE1','TCE1','SLLPRI','TCPRI','SEDEIP','CONECT','ACCESO') )";
                                ...
                                $concepto=" and b.CONCEPTO_ID='$concepto' and b.TIPO_ELEMENTO_ID IN('ACCESP','INSIP','INSHFC','TO','TOIP') and b.UEN_CALCULADA ='HG' ";
			...
			$sqlupdate="update informe_petec_pendientesm set ASESOR='$user',VIEWS=VIEWS+1 where PEDIDO_ID = '$mypedido' and STATUS='PENDI_PETEC'";
                        $x = $this->mysqli->query($sqlupdate);


			$query1="SELECT b.ID,b.PEDIDO_ID,b.SUBPEDIDO_ID,b.SOLICITUD_ID,b.TIPO_ELEMENTO_ID,b.PRODUCTO,b.UEN_CALCULADA,b.ESTRATO,b.MUNICIPIO_ID,b.DIRECCION_SERVICIO,b.PAGINA_SERVICIO,CAST(TIMEDIFF(CURRENT_TIMESTAMP(),(b.FECHA_ESTADO)) AS CHAR(255)) as TIEMPO_COLA,b.FUENTE,b.CONCEPTO_ID,b.FECHA_ESTADO,b.USUARIO_BLOQUEO_FENIX,b.TIPO_TRABAJO,b.CONCEPTO_ANTERIOR from informe_petec_pendientesm b where b.PEDIDO_ID = '$mypedido' and b.STATUS='PENDI_PETEC' $concepto ";
		...
		private function listadoTransaccionesNCA(){
			...
			$query="SELECT count(*) as counter from tbl_usuarios";
                        ...
                        if($r->num_rows > 0){
                                ...
                                while($row = $r->fetch_assoc()){
                                        $result[] = $row;
					$result['USUARIO_NOMBRE']=utf8_encode($result['USUARIO_NOMBRE']);
                                }
                                $this->response($this->json(array($result,$counter)), 200); // send user details
                        }
                        $this->response('',204);        // If no records "No Content" status
