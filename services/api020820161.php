...
//Inicia Mundo Agendamiento - Reagendamiento               
                ...
                private function csvHistoricosAgendamiento(){
                        ...
                        $query=" SELECT ".
                        " PEDIDO_ID,CONCEPTOS,ACTIVIDADES,NOVEDAD,FECHA_CITA_FENIX,FECHA_CITA_REAGENDA,JORNADA_CITA,FECHA_INGRESO,FECHA_CARGA,ASESOR,FECHA_INICIO,FECHA_FIN,FECHA_ESTADO,DURACION,OBSERVACION_FENIX,OBSERVACION_GESTOR,FUENTE,ACTIVIDAD_GESTOR,ASESORNAME,CELULAR_AVISAR,CLIENTE_ID,CORREO_UNE,DIRECCION_ENVIO,E_MAIL_AVISAR,MICROZONA,NOMBRE_USUARIO,PARENT_ID,TELEFONO_AVISAR,TIEMPO_TOTAL,PROGRAMACION,SOURCE ".
                        " from gestor_historicos_reagendamiento where ".
		...
		private function csvPreInstalaciones(){
		 ...
		 private function insertPedidoReconfiguracion(){
			...
			if($novedad=='AGENDADO'||$novedad=='AGENDADO MANUAL'||$novedad=='AGENDADO_FUTURO'|| $novedad=='NO CONOCE EL PEDIDO'){
				...
				$sqlupdate="update gestor_pendientes_reagendamiento set FECHA_ACTUALIZACION='$today',STATUS='PENDI_AGEN',FECHA_CITA_REAGENDA='$fecha_cita_reagen',TODAY_TRIES=(SELECT COUNT(*) FROM gestor_historicos_reagendamiento WHERE PEDIDO_ID='$PEDIDO_ID'),PROGRAMACION='$programacion',ASESOR='' WHERE ID=$parent ";
			}
			//echo $sqlupdate;
    ...
    return "No rows!!!!";
		...
		private function pedidosPorUser(){
                ...
                private function pedidosPorUserReagendamiento(){
                        ...
                        $id = $this->_request['userID'];
                        $today = date("Y-m-d");
                        $query="SELECT ID,PEDIDO_ID,FUENTE,NOVEDAD,FECHA_FIN,DURACION from gestor_historicos_reagendamiento where ASESOR='$id' and FECHA_FIN between '$today 00:00:00' and '$today 23:59:59'";
...
*/
		...
		private function csvPendientes(){
			...
			$query="SELECT a.PEDIDO_ID,a.PEDIDO,a.SUBPEDIDO_ID,a.SOLICITUD_ID,a.TIPO_ELEMENTO_ID,a.PRODUCTO,a.UEN_CALCULADA,a.ESTRATO,MUNICIPIO_ID,a.PAGINA_SERVICIO,CAST(TIMEDIFF(CURRENT_TIMESTAMP(),(a.FECHA_ESTADO)) AS CHAR(255)) as TIEMPO_COLA, hour(TIMEDIFF(CURRENT_TIMESTAMP(),(a.FECHA_ESTADO))) as TIEMPO_HORAS,a.FUENTE,a.CONCEPTO_ID,a.FECHA_ESTADO,a.FECHA_INGRESO, a.FECHA_CITA,a.STATUS,a.RADICADO_TEMPORAL from informe_petec_pendientesm a where (a.STATUS='PENDI_PETEC' or a.STATUS='MALO') $concepto ";
			...
			$r = $this->mysqli->query($query) or die($this->mysqli->error.__LINE__);
                        ...
                        if($r->num_rows > 0){
                                $result = array();
				$fp = fopen("../tmp/$filename", 'w');
				fputcsv($fp, array('PEDIDO_ID','PEDIDO','SUBPEDIDO_ID','SOLICITUD_ID','TIPO_ELEMENTO_ID','PRODUCTO','UEN_CALCULADA','ESTRATO','MUNICIPIO_ID','PAGINA_SERVICIO','TIEMPO_COLA','TIEMPO_HORAS','FUENTE','CONCEPTO_ID','FECHA_ESTADO','FECHA_INGRESO','FECHA_CITA','STATUS','RADICADO_TEMPORAL'));
                                while($row = $r->fetch_assoc()){
                                        $result[] = $row;
					fputcsv($fp, $row);
                                }
                ...
                private function csvPendientesAgendamiento(){
                       ...
                       " WHERE pm.STATUS IN ('PENDI_AGEN',  'MALO') ";
                        ...
                        if($r->num_rows > 0){
                                $result = array();
                                $fp = fopen("../tmp/$filename", 'w');
                                fputcsv($fp, array('PEDIDO_ID','FECHA_INGRESO','FECHA_ESTADO','FUENTE','STATUS','ULTIMA_NOVEDAD','MICROZONA','ZONA','OBSERVACION','PROGRAMACION'));
                                while($row = $r->fetch_assoc()){
                ...
                private function csvMalosAgendamiento(){
                       ...
                       "     WHERE a.PEDIDO_ID =  pm.PEDIDO_ID) )AS ULTIMA_NOVEDAD ". 
                        ...
                        if($r->num_rows > 0){
                                $result = array();
                                $fp = fopen("../tmp/$filename", 'w');
                                fputcsv($fp, array('PEDIDO_ID','FECHA_INGRESO','FECHA_ESTADO','FUENTE','STATUS','ULTIMA_NOVEDAD','MICROZONA','ZONA','OBSERVACION'));
                                while($row = $r->fetch_assoc()){
                                        $row['ULTIMA_NOVEDAD']=utf8_decode($row['ULTIMA_NOVEDAD']);
                                        $row['OBSERVACION_FENIX']=str_replace(array("\n","\r"), '/', $row['OBSERVACION_FENIX']);
                                        $result[] = $row;
	...
	private function buscarPedidoAgendamientoRegistro(){//pendientes
			...
			$query="SELECT count(*) as counter from informe_petec_pendientesm a where (a.STATUS='PENDI_PETEC' or a.STATUS='MALO') $concepto ";
                ...
                private function vecinosPagina(){
						...
						"   order by FN_VALOR_CARACT_IDENTIF(I.IDENTIFICADOR_ID, '35') asc";						
					//echo $query;
					//var_dump($query);
          ...
          private function buscarPedido(){
			...
			//1. CIERRO TODO LO QUE ESTE ABIERTO DE ESTE PEDIDO EN EL GESTOR..
			//
			//
 ...
 $sqlfeed="insert into activity_feed(user,user_name, grupo,status,pedido_oferta,accion,concepto_id) values ('$user','$username','','','PEDIDO: $pedido','BUSCARPEDIDO','') ";
		...
		private function buscarPedidoFenix($pedido_id){
                        ...
                        $sqlfenix="SELECT  ".
								...
								" WHERE ".
								"       SOL.PEDIDO_ID='$pedido_id' ".
								"       and SOL.TIPO_ELEMENTO_ID IN ('BDID', 'TDID','BDIDE1', 'TDIDE1', 'BDODE1', 'TDODE1', 'TO', 'TOIP','INSHFC', 'INSIP', 'INSTIP', 'SEDEIP', 'P2MB', '3PLAY', 'CNTXIP', 'ACCESP', 'PLANT', 'PLP', 'PTLAN', 'PMULT', 'PPCM', 'PBRI', 'PPRI', 'INSTA', 'TP', 'PBRI','SLL', 'TC', 'SLLBRI', 'TCBRI', 'SLLPRI', 'TCPRI','SEDEIP','SEDECX','EQURED','STBOX','ACCESO','EQACCP','INTCON','TELEV') ".
...
*/
		...
		//DemePedidoPetec Principal, Asignaciones - Reconfiguracion.
			...
			if($concepto=="PETEC"){
				...
				$concepto=" and b.CONCEPTO_ID='$concepto' and b.UEN_CALCULADA ='HG' AND b.PROGRAMACION='' ";
			...
			$sqlupdate="update informe_petec_pendientesm set ASESOR='$user',PROGRAMACION='',VIEWS=VIEWS+1,FECHA_VISTO_ASESOR='$fecha_visto' where PEDIDO_ID = '$mypedido' and (STATUS='PENDI_PETEC'||STATUS='BUSCADO_PETEC')";
            $x = $this->mysqli->query($sqlupdate);

			$query1="	SELECT b.ID,b.PEDIDO_ID,b.SUBPEDIDO_ID,b.SOLICITUD_ID,b.TIPO_ELEMENTO_ID,b.PRODUCTO, b.PRODUCTO_ID,	".
					"	b.UEN_CALCULADA,b.ESTRATO,b.MUNICIPIO_ID,b.DIRECCION_SERVICIO,b.PAGINA_SERVICIO,	".
					"	CAST(TIMEDIFF(CURRENT_TIMESTAMP(),(b.FECHA_ESTADO)) AS CHAR(255)) as TIEMPO_COLA,	".
					...
					"	where b.PEDIDO_ID = '$mypedido' and b.STATUS='PENDI_PETEC' $concepto ";
			...
			$llamadaReconfiguracion="0";	
                        ...
                        $departamento=" and b.TECNOLOGIA_ID='".utf8_decode($departamento)."' ";
                        }	else{
			...
			" and b.CONCEPTOS NOT LIKE '%REAGE%' ".
                        ...
                        $zona.
                        $microzona.
			" and (select count(*) from gestor_historicos_reagendamiento a where a.PEDIDO_ID=b.PEDIDO_ID and a.FECHA_FIN between '$today 00:00:00' and '$today 23:59:59') <=3 ".
			...
			" and (select count(*) from gestor_historicos_reagendamiento a where a.PEDIDO_ID=b.PEDIDO_ID and FECHA_FIN between '$today 00:00:00' and '$today 23:59:59') <=3 ".
			$departamento.
			$zona.
			$microzona.
			//$plaza.
			...
			unlink($filename);
                ...
                private function listadoPedidosAgendamiento(){//historico por 1 pedido agendamiento
                        ...
                        $page=$page*100;
                        //counter
                        $query="SELECT count(*) as counter from gestor_historicos_reagendamiento where fecha_fin between '$fechaini 00:00:00' and '$fechafin 23:59:59'";
                        $rr = $this->mysqli->query($query) or die($this->mysqli->error.__LINE__);
                        $counter=0;
                        if($rr->num_rows > 0){
                                $result = array();
                        ...
                        $query="SELECT id, pedido_id, fuente, actividad_gestor, fecha_fin, duracion, novedad, asesor,fecha_cita_reagenda,conceptos,actividades from gestor_historicos_reagendamiento where fecha_fin between '$fechaini 00:00:00' and '$fechafin 23:59:59' order by fecha_fin desc limit 100 offset $page";
                ...
                private function buscarPedidoAgendamiento(){
                        ...
                        $query1="SELECT b.ID as PARENT_ID,b.PEDIDO_ID,b.FECHA_CITA_FENIX,b.CLIENTE_ID,b.CELULAR_AVISAR,b.CORREO_UNE,b.DIRECCION_ENVIO,b.E_MAIL_AVISAR,b.NOMBRE_USUARIO,b.FECHA_INGRESO,b.TELEFONO_AVISAR,b.CONCEPTOS,b.ACTIVIDADES,cast(TIMESTAMPDIFF(HOUR,(b.FECHA_INGRESO),CURRENT_TIMESTAMP())/24 AS decimal(5,2)) as TIEMPO_TOTAL,b.MICROZONA,b.OBSERVACION_FENIX,b.FUENTE,b.TODAY_TRIES,b.PROGRAMACION,b.FECHA_ESTADO,(select a.NOVEDAD from gestor_historicos_reagendamiento a where a.PEDIDO_ID='$pedido' order by a.ID desc limit 1) as HISTORICO_NOVEDAD from gestor_pendientes_reagendamiento b where b.PEDIDO_ID = '$pedido' order by b.ID desc limit 1 ";
                       ...
                       $tname1 = basename( $_FILES["fileUpload"]["name"]);
			            ...
			            for($v=$start_v; $v<=$end_v; $v++){
			                ...
			                for($h=$start_h; ord($h)<=ord($end_h);$this->pp($h)){
								...
								$FECHA_FIN = gmdate("d-m-Y",$timestamp);//fecha formateada+
							  	$table .= "<td>";
							  	}
			                	if($h=="I"){
			                	$NOVEDAD=$cellValue;
			              ...
			              $guardar=rtrim($guardar,',');
                        	...
                        	$sqlbodega_datos="insert into portalbd.gestor_bodega_bodega_datos (CAMPANAID,LANZAMIENTO,IDLLAMADA1,TELEFONO,MENSAJE,ACCION,FECHA,IDLLAMADA2,ESTADO,CEDULA,DETALLE,PEDIDO,CODIGO_RESULTADO,FECHA_AGENDA,JORNADA_AGENDA,CAUSA,MUNICIPIO,ZONA,TIPO_TRANSACCION,NOMBRE_CLIENTE,DEPARTAMENTO,EMAIL,FECHA_ENVIO,HORA_ENVIO,INTERFAZ,ACCESO) values ($guardar) ";  
					     	...
					     	$sqldatos="insert into portalbd.gestor_historicos_reagendamiento (PEDIDO_ID,CLIENTE_ID,ACCESO,FUENTE,FECHA_FIN,ASESOR,NOVEDAD,OBSERVACION_GESTOR) values ('$PEDIDO_ID','$cliente_id','$ACCESO','$FUENTE','$FECHA_FIN','$ASESOR','$NOVEDAD','OBSERVACION')";
                        	//echo  $user;
                        ...
                        if ($tname1 == "IMPORTANTES.xlsx"){
                        	...
                        	$rst = $this->mysqli->query($pend);
                        		...
                        		if ($rst->num_rows > 0){
		                        	...
		                        	$sqlemail="insert into portalbd.gestor_pendientes_reagendamiento (PEDIDO_ID,CONCEPTOS,CLIENTE_ID,NOMBRE_USUARIO,DEPARTAMENTO,DIRECCION_ENVIO,FUENTE) values ($guardar) ";
	...
	//	echo $query;
            ...
            $r = $conna->query($query) or die($this->mysqli->error.__LINE__);
                        ...
                        if($r->num_rows > 0){
                                $result = array();
                                 $fp = fopen("../tmp/$filename", 'w');
                                 fputcsv($fp, array('FECHA','DEPARTAMENTO_ID','ZONA','MICROZONA','SEGMENTO','PARAM_AM','DISP_AM','PARAM_PM','DISP_PM','PARAM_HF','DISP_HF','TOTAL_DISP'));
                                while($row = $r->fetch_assoc()){
                                        fputcsv($fp, $row);
