		...
		private function csvPreInstalaciones(){
		 ...
		 private function insertPedidoReconfiguracion(){
			...
			if($novedad=='AGENDADO'||$novedad=='AGENDADO MANUAL'||$novedad=='AGENDADO_FUTURO'|| $novedad=='NO CONOCE EL PEDIDO'){
				...
				$sqlupdate="update gestor_pendientes_reagendamiento set FECHA_ACTUALIZACION='$today',STATUS='PENDI_AGEN',FECHA_CITA_REAGENDA='$fecha_cita_reagen',TODAY_TRIES=(SELECT COUNT(*) FROM gestor_historicos_reagendamiento WHERE PEDIDO_ID='$PEDIDO_ID' and FECHA_CARGA BETWEEN '$today2 00:00:00' AND '$today2 23:59:59'),PROGRAMACION='$programacion',ASESOR='' WHERE ID=$parent ";
			}
			//echo $sqlupdate;
                       ...
                       if($this->get_request_method() != "GET"){
                        ...
                        $dep = $this->_request['departamento'];
                           ...
                           $sql= " SELECT DISTINCT ZONA".
                            " FROM alistamiento.departamentos_zonas ".
...
//****************
                private function listadoParametrizadosSiebel(){//parametrizados
                	...
                	private function csvParametrizacionSiebel(){
                        ...
                        " from alistamiento.parametrizacion_siebel ".
                        " WHERE FECHA >= '$today' and(AM is not null or PM is not null) ".
                     ...
                     private function csvParametrizacionMicrozona(){
                        ...
                        " from alistamiento.parametrizacion_siebel ".
                        " WHERE FECHA >= '$today' ".
                        " and DEPARTAMENTO = '$depa' and ZONA = '$zona' ".
 ...
 $sqlfeed="insert into activity_feed(user,user_name, grupo,status,pedido_oferta,accion,concepto_id) values ('$user','$username','','','PEDIDO: $pedido','BUSCARPEDIDO','') ";
		...
		private function buscarPedidoFenix($pedido_id){
                        ...
                        $sqlfenix="SELECT   ".
                                ...
                                "    , convert((FNX_TRABAJOS_SOLICITUDES.TIPO_TRABAJO || '-' || FN_NOMBRE_CARACTERISTICA(FNX_TRABAJOS_SOLICITUDES.CARACTERISTICA_ID)),'US7ASCII') as DESC_TIPO_TRABAJO ".
                                ...
                                "        WHERE ".
                                "         SOL.PEDIDO_ID='$pedido_id' ".
                                "         AND (FNX_TRABAJOS_SOLICITUDES.CARACTERISTICA_ID IN ('1', '35', '56', '124','33','4208','2050','200','49','2787') ".
                                "         or FNX_TRABAJOS_SOLICITUDES.CARACTERISTICA_ID is null) ".
...
*/
		...
		//DemePedidoPetec Principal, Asignaciones - Reconfiguracion.
			...
			$llamadaReconfiguracion="0";	
                        ...
                        if($departamento=="VACIOS") $departamento="";
			...
			unlink($filename);
                ...
                private function pedidosPorPedidoAgendamiento(){//historico por 1 pedido
                        ...
                        $pedido = $this->_request['pedido'];
                        $today = date("Y-m-d");
                        $query="SELECT ID,FUENTE,NOVEDAD,FECHA_FIN,DURACION,ASESOR,OBSERVACION_GESTOR from gestor_historicos_reagendamiento where PEDIDO_ID = '$pedido' order by FECHA_FIN desc limit 10";
                        $r = $this->mysqli->query($query) or die($this->mysqli->error.__LINE__);
                ...
                private function cargar_datos(){               	
                       ...
                       // var_dump($_FILES);
			            ...
			            for($v=$start_v; $v<=$end_v; $v++){
			               ...
			               $julian=rtrim($julian,',');
			               $sqlemail="insert into portalbd.gestor_bodega_agendamiento (PEDIDO,CEDULA,NOMBRE_CLIENTE,DEPARTAMENTO,ZONA,EMAIL,FECHA_ENVIO,HORA_ENVIO,CANAL,INTERFAZ,CODIGO_RESULTADO) values ($julian) ";
			              // echo  $sqlemail."\n";
			               $rrrr = $this->mysqli->query($sqlemail) or die($this->mysqli->error.__LINE__);
			               ...
			               //echo $julian;
			             //$sqlemail="insert into portalbd.gestor_bodega_agendamiento (PEDIDO,CEDULA,NOMBRE_CLIENTE,DEPARTAMENTO,ZONA,EMAIL,FECHA_ENVIO,HORA_ENVIO,CANAL,INTERFAZ,CODIGO_RESULTADO) values ($julian) ";
                        // $sqlemail="insert into gestor_bodega_agendamiento (PEDIDO,CEDULA,NOMBRE_CLIENTE,DEPARTAMENTO,ZONA,EMAIL,FECHA_ENVIO,HORA_ENVIO,CANAL,INTERFAZ,CODIGO_RESULTADO) values ('$PEDIDO_ID','$cliente_id','$nombre_usuario','$departamento','$zona','$email','$FECHA_ENVIO','$HORA_ENVIO','$CANAL','$INTERFAZ','$CODIGO_RESULTADO') ";	
                      // $rrrr = $this->mysqli->query($sqlemail) or die($this->mysqli->error.__LINE__);
