...
//---------------------insert pedido reagendamiento--------------------agendamiento-------------
    ...
    private function insertPedidoReagendamiento(){
        ...
        if($novedad=='AGENDADO'||$novedad=='AGENDADO MANUAL'|| $novedad=='AGENDADO_FUTURO'|| $novedad=='CONFIRMA SOLUCION'
            ...
            ||$novedad=='YA ESTA CUMPLIDO–CERRADO'||$novedad=='YA ESTA CUMPLIDO–PENDIENTE' ||$novedad=='ANULADO POR ASESOR' || $novedad== 'ANULADO CERRADO Y/O PENDIENTE' || $novedad== 'CUMPLIDO CERRADO Y/O PEDIENTE' ||$novedad== 'ERROR SIEBEL' || $novedad=='YA ESTA CUMPLIDO' || $novedad=='NO SUBIO A CLICK' || $novedad=='PEDIDO EXCEPCIONADO'){
...
//-----------------------fin listado activacion
    ...
    private function buscarPedidoFenix($pedido_id){
        ...
        $sqlfenix="SELECT  ".
            ...
            " WHERE ".
            "       SOL.PEDIDO_ID='$pedido_id' ".
            "       and SOL.TIPO_ELEMENTO_ID IN ('BDID', 'TDID','BDIDE1', 'TDIDE1', 'BDODE1', 'TDODE1', 'TO', 'TOIP','INSHFC', 'INSIP', 'INSTIP', 'SEDEIP', 'P2MB', '3PLAY', 'CNTXIP', 'ACCESP', 'PLANT', 'PLP', 'PTLAN', 'PMULT', 'PPCM', 'PBRI', 'PPRI', 'INSTA', 'TP', 'PBRI','SLL', 'TC', 'SLLBRI', 'TCBRI', 'SLLPRI', 'TCPRI','SEDEIP','SEDECX','STBOX','ACCESO','DECO','EQURED') ".
            "       AND SOL.CONCEPTO_ID IN ('PETEC','PSIEB', 'PUMED', 'PRESI', 'PECBA', '19', 'O-106', 'O-13', 'O-15','65') ".
            "       AND SOL.SUBPEDIDO_ID=FNX_SUBPEDIDOS.SUBPEDIDO_ID  ".
            "       AND SOL.PEDIDO_ID=FNX_SUBPEDIDOS.PEDIDO_ID  ".
            "       AND SOL.PEDIDO_ID=FNX_PEDIDOS.PEDIDO_ID  ".
    ...
    private function csvNCA(){
        ...
        $query=" SELECT ".
            "pedido_id AS OFERTA,MUNICIPIO_ID, TIPO_TRABAJO,ESTADO,FECHA_INICIO,FECHA_FIN,concepto_final as ESTADO_FINAL,
		     DURACION,INCIDENTE,OBSERVACIONES_PROCESO, user AS USUARIO ".
            " from pedidos where ".
        ...
        if($r->num_rows > 0){
            $result = array();
            $fp = fopen("../tmp/$filename", 'w');
            fputcsv($fp, array('OFERTA','MUNICIPIO_ID','TIPO_TRABAJO','ESTADO','FECHA_INICIO','FECHA_FIN','ESTADO_FINAL','DURACION','INCIDENTE','OBSERVACIONES_PROCESO','USUARIO'));
            while($row = $r->fetch_assoc()){
                //$result[] = $row;
                fputcsv($fp, $row);
            }
