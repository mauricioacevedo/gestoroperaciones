    ...
    private function insertTransaccionsiebelactivacion(){
        ...
        if(!empty($pedido)){
            ...
            if($tabla=='ACTIVADOR_SUSPECORE' ){
                ...
                if($TIPIFICACION=='FINALIZADA' || $TIPIFICACION=='RENUMERAR'){
                    $sqlupdate="update pendientes_amarillas set FECHA_CARGA = '$today',STATUS='CERRADO_ACTI',FECHA_EXCEPCION = '$FECHA_EXCEPCION' WHERE  STATUS='PENDI_ACTI'";
                    //  echo $sqlupdate;
                }else {

                    $sqlupdate="update pendientes_amarillas set FECHA_CARGA = '$today',STATUS='MALO',FECHA_EXCEPCION = '$FECHA_EXCEPCION' WHERE STATUS='PENDI_ACTI'";
                }
            }
            // echo $sqlupdate;
            $rr = $this->mysqli->query($sqlupdate) or die($this->mysqli->error.__LINE__);
                ...
                if ($ldapbind) {
                    ...
                    if($cantdata>0){
                        ...
                        for ($i=0; $i<$data["count"]; $i++) {
                            ...
                            $object->USUARIO_NOMBRE = $this->quitar_tildes(strtoupper($data[$i]["displayname"][0]));
