  ...
  //      $TABLA = " from gestor_activacion_pendientes_activador_dom b " ;
       ...
       // echo $query1;
        ...
        $query1= " SELECT ".
                ...
                " ,b.PRODUCTO,b.ACTIVIDAD,b.FUENTE,b.GRUPO,b.OBSERVACION".
...
//-----------------------insertactivacion----------------
    ...
    private function insertTransaccionsiebelactivacion(){
        ...
        $pedido = json_decode(file_get_contents("php://input"),true);
        //var_dump($pedido);
        $column_names = array('ORDER_SEQ_ID','PEDIDO','REFERENCE_NUMBER','ESTADO','FECHA_CREACION','TAREA_EXCEPCION','FECHA_EXCEPCION','PRODUCTO','IDSERVICIORAIZ','TRANSACCION','CODIGO_CIUDAD','STATUS','ASESOR','FECHA_GESTION','TIPIFICACION','FECHA_INICIO','FECHA_FIN','DURACION','OBSERVACION','NUMERO_CR','TABLA');
        $pedido=$pedido['pedido'];
        $keys = array_keys($pedido);
        $today = date("Y-m-d H:i:s");
        $today2 = date("Y-m-d");
        ...
        if(!empty($pedido)){
            ...
            $query = "insert into gestor_historico_activacion (ORDER_SEQ_ID,PEDIDO,REFERENCE_NUMBER,ESTADO,FECHA_CREACION,TAREA_EXCEPCION,FECHA_EXCEPCION,PRODUCTO,IDSERVICIORAIZ,TRANSACCION,CODIGO_CIUDAD,ASESOR,FECHA_GESTION,TIPIFICACION,FECHA_INICIO,FECHA_FIN,DURACION,OBSERVACION,NUMERO_CR,TABLA) values ('$ORDER_SEQ_ID','$PEDIDO','$REFERENCE_NUMBER','$ESTADO','$FECHA_CREACION','$TAREA_EXCEPCION','$FECHA_EXCEPCION','$PRODUCTO','$IDSERVICIORAIZ','$TRANSACCION','$CODIGO_CIUDAD','$ASESOR','$today','$TIPIFICACION','$FECHA_INICIO','$FECHA_FIN','$DURACION','$OBSERVACION','$NUMERO_CR','$tabla') ";
    ...
    private function listarBuscarPedidoAuditoriaAsignaciones()
    {
       ...
       // echo $sql;
        ...
        if($r->num_rows > 0){
            $result = array();
            while($row = $r->fetch_assoc()){
                $row['OBSERVACIONES']=utf8_dencode($row['OBSERVACIONES']);
                $result[] = $row;
            }
