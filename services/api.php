...
//-----------------------exportar historico agendamiento-----------------------agendamiento
    ...
    private function csvHistoricosAgendamiento(){
        ...
        $query=" SELECT ".
            ...
            " ,a.FUENTE,a.ACTIVIDAD_GESTOR,a.ASESORNAME ".
        ...
        if($r = $this->mysqli->use_result()){
            ...
            fputcsv($fp, array( 'PEDIDO_ID','CONCEPTOS','ACTIVIDADES','NOVEDAD','FECHA_CITA_FENIX','FECHA_CITA_REAGENDA','JORNADA_CITA','FECHA_INGRESO','FECHA_CARGA','ASESOR','FECHA_INICIO','FECHA_FIN','FECHA_ESTADO','DURACION','OBSERVACION_FENIX','FUENTE','ACTIVIDAD_GESTOR','ASESORNAME','CELULAR_AVISAR','CLIENTE_ID','CORREO_UNE','DIRECCION_ENVIO','E_MAIL_AVISAR','MICROZONA','NOMBRE_USUARIO','PARENT_ID','TELEFONO_AVISAR','TIEMPO_TOTAL','PROGRAMACION','SOURCE','DEPARTAMENTO','ACCESO','NUMERO_CR','IDLLAMADA','SUBZONA_ID','PROCESO','OBSERVACION_GESTOR','INTENTOS DE CONTACTO','TECNOLOGIA'),chr (124));
...
//-----------------------------fin extortar activacion GTC---------------------------------
    ...
    private function csvListadoActivacion(){
        ...
        $query= "SELECT ORDER_SEQ_ID,PEDIDO, ESTADO, FECHA_CREACION, FECHA_EXCEPCION,TRANSACCION, ".
            " PRODUCTO,ASESOR,FECHA_EXPORTE,HORA_EXPORTE,FECHA_GESTION,HORA_GESTION, ".
            " TIPIFICACION,FECHA_INICIO,FECHA_FIN,OBSERVACION,PSR,NUMERO_PSR,TABLA ".
        ...
        if($r->num_rows > 0){
            ...
            fputcsv($fp, array( 'ORDER_SEQ_ID','PEDIDO','ESTADO','FECHA_CREACION','FECHA_EXCEPCION','TRANSACCION','PRODUCTO','ASESOR','FECHA_EXPORTE','HORA_EXPORTE','FECHA_GESTION','HORA_GESTION','TIPIFICACION','FECHA_INICIO','FECHA_FIN','OBSERVACION','PSR','NUMERO_PSR','TABLA','DURACION'));
...
//-------------------------------------insertar pedido ---------asignacion---------
    ...
    private function insertMPedido(){
        ...
        if($estado_id == "ENRRUTADO" && $estado == "RC-SIEBEL"){
                ...
                $query = " update informe_petec_pendientesm set CONCEPTO_ID = '$estado',  CONCEPTO_ANTERIOR = '$estado',  STATUS = 'PENDI_PETEC' ".
...
//------------------------listado activacion-----------------
    ...
    private function listadoactivacion(){
        ...
        $query= "SELECT ORDER_SEQ_ID,PEDIDO, ESTADO, FECHA_CREACION, FECHA_EXCEPCION,TRANSACCION ".
            " , PRODUCTO,ASESOR,FECHA_EXPORTE,HORA_EXPORTE,FECHA_GESTION,HORA_GESTION ".
            " ,TIPIFICACION,FECHA_INICIO,FECHA_FIN,TABLA ".
            " ,my_sec_to_time(timestampdiff(second,fecha_inicio,fecha_fin)) as DURACION ".
            " from gestor_historico_activacion ".
            " where fecha_fin between '$fechaini 00:00:00' ".
            " and '$fechafin 23:59:59'  order by fecha_fin desc limit 100 offset $page";
...
//-----------------------fin listado activacion
    ...
    private function buscarPedido(){
       ...
       /* $pedidoid = $this->_request['pedidoid'];
        $pedidoid1 = $this->_request['ID'];

        echo var_dump ($pedidoid);
        echo var_dump ($pedidoid1);*/
...
//función para buscar pedidos en fenix Bogotá que no estan en la base de datos.
    ...
    private function updateParametro(){
        ...
        $sql="UPDATE gestor_parametros ".
            " SET VALOR='$value',USUARIO_ID='$user',ULTIMA_ACTUALIZACION='$time' ".
...
/*       $parametroAsesor = " select PARAMETRO_ENTREGA from tbl_usuarios where USUARIO_ID in ('$user') ";
        ...
        if($concepto=="PETEC"){
            ...
            $concepto=" and b.CONCEPTO_ID in ('PETEC') and (b.TIPO_ELEMENTO_ID IN ('E2MB','P2MB','INSTIP','CNTXIP','SEDECX','PLANT','PLP','PTLAN','MTLAN', 'PMULT','EPCM','PPCM','PBRI','PPRI','TV','TP','BDID','TDID','BDIDE1','TDIDE1','BDODE1','TDODE1','SLL','TC','SLLBRI','TCBRI','SLLE1','TCE1','SLLPRI','TCPRI','SEDEIP','CONECT','ACCESO','2MB','INSTA','TRKSIP') )";
        ...
        if($mypedido==""){
            ...
            if($rr->num_rows > 0){//recorro los registros de la consulta para
                ...
                $query1="select b.CLIENTE_ID,b.PEDIDO_ID,b.SUBPEDIDO_ID,b.SOLICITUD_ID,b.FECHA_ESTADO,b.FECHA_INGRESO, b.FECHA_CITA, b.TIPO_ELEMENTO_ID ".
                    ...
                    //" AND b.MUNICIPIO_ID IN (select a.MUNICIPIO_ID from tbl_plazas a where a.PLAZA='$plaza') ".
                    //" order by b.$parametroBusqueda2
					" order by b.FECHA_CITA, b.FECHA_ESTADO $parametroOrden ";
                    //echo "ingreso 2";
    ...
    private function cargar_datos_activacion(){
        ...
        for($v=$start_v; $v<=$end_v; $v++){
            ...
            if ($tname1 <> ""){
                ...
                $sqldatos="insert into gestor_historico_activacion (FECHA_EXCEPCION,HORA,PEDIDO,PRODUCTO,TRANSACCION,APLICATIVO,OBSERVACION,NUMERO_CR,TIPIFICACION,ASESOR,PEDIDO_FENIX,TABLA,FECHA_EXPORTE,HORA_EXPORTE,FECHA_GESTION,HORA_GESTION,SOURCE,FECHA_INICIO,FECHA_FIN) values ($guardar,'MANUAL','$today','$today'                )";
                //  echo  $sqldatos;
                $r = $this->mysqli->query($sqldatos) or die($this->mysqli->error.__LINE__);
...
//------------------------Listado concepts sistema
    ...
    private function gestorConceptos(){
        ...
        {
            $grupo = "in ('ASIGNACIONES','POBLACIONES','RECONFIGURACION','OPEN_PEREIRA') ";
            $query=	" SELECT * FROM portalbd.gestor_conceptos where GRUPO $grupo ";
        }
        ...
        if($gestion['gestion']['source']=="MANUAL"){
...

            {
                ...
                $queryReconf = "update informe_petec_pendientesm set CONCEPTO_ID = 'RC-SIEBEL', STATUS = 'PENDI_PETEC'".
                                " ,OBSERVACIONES = '$observacionAsesor', FECHA_ESTADO ='$fechaServidor' WHERE ID = '$idpedido' ";
                $insertReconf = $this->mysqli->query($queryReconf);
                //echo var_dump("ingreso");
            }
