...
//-------------------calcular detalle TMA----------------------------------asignaciones-----------------
    ...
    private function csvPendientes(){
        ...
        if($r->num_rows > 0){
            $result = array();
            $fp = fopen("../tmp/$filename", 'w');
            fputcsv($fp, array('PEDIDO_ID','PEDIDO','PEDIDO_CRM','SUBPEDIDO_ID','SOLICITUD_ID','PROGRAMACION','TIPO_TRABAJO','TIPO_TRABAJO_ORIGINAL','TIPO_ELEMENTO_ID','PRODUCTO','PRODUCTO_ID','UEN_CALCULADA','ESTRATO','MUNICIPIO_ID','PAGINA_SERVICIO','DIRECCION_SERVICIO','TIEMPO_COLA','TIEMPO_INGRESO','FUENTE','CONCEPTO_ID','FECHA_ESTADO','FECHA_INGRESO','FECHA_CITA','STATUS','MOTIVO_MALO','INCIDENTE','RADICADO_TEMPORAL','PEDIDOFNX', 'CONCEPTO_CRM','ZONA'));
            while($row = $r->fetch_assoc()){
