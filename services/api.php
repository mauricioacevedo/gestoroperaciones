    ...
    private function insertTransaccionsiebelactivacion(){
       ...
       // $OBSERVACIONES_PROCESO=$pedido['OBSERVACIONES_PROCESO'];
        ...
        if(!empty($pedido)){
            ...
            $query = "insert into gestor_historico_activacion (ORDER_SEQ_ID,PEDIDO,REFERENCE_NUMBER,ESTADO,FECHA_CREACION,TAREA_EXCEPCION,FECHA_EXCEPCION,PRODUCTO,IDSERVICIORAIZ,TRANSACCION,CODIGO_CIUDAD,ASESOR,FECHA_GESTION,TIPIFICACION,FECHA_INICIO,FECHA_FIN,DURACION,OBSERVACION,NUMERO_CR,TABLA,ESTADO_ID,OBSERVACIONES_PROCESO) values ('$ORDER_SEQ_ID','$PEDIDO','$REFERENCE_NUMBER','$ESTADO','$FECHA_CREACION','$TAREA_EXCEPCION','$FECHA_EXCEPCION','$PRODUCTO','$IDSERVICIORAIZ','$TRANSACCION','$CODIGO_CIUDAD','$ASESOR','$today','$TIPIFICACION','$FECHA_INICIO','$FECHA_FIN','$DURACION','$OBSERVACION','$NUMERO_CR','$tabla') ";

             //var_dump($OBSERVACIONES_PROCESO);
            $r = $this->mysqli->query($query) or die($this->mysqli->error.__LINE__);
