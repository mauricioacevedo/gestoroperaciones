...
//----------------------------exportar historico agendamiento edatel-----------------agendamiento
    ...
    private function csvHistoricosAgendamientoEdatel(){
        ...
        $filename="Agendamiento-Fenix_NAL-$login-$today.csv";
        $query=" SELECT ".
            " PEDIDO_ID,CONCEPTOS,ACTIVIDADES,NOVEDAD,FECHA_CITA_FENIX,FECHA_CITA_REAGENDA,JORNADA_CITA ".
            ",FECHA_INGRESO,FECHA_CARGA,ASESOR,FECHA_INICIO,FECHA_FIN,FECHA_ESTADO,DURACION,OBSERVACION_FENIX ".
            ",OBSERVACION_GESTOR,FUENTE,ACTIVIDAD_GESTOR,ASESORNAME,CELULAR_AVISAR,CLIENTE_ID,CORREO_UNE ".
            ",DIRECCION_ENVIO,E_MAIL_AVISAR,MICROZONA,NOMBRE_USUARIO,PARENT_ID,TELEFONO_AVISAR,TIEMPO_TOTAL ".
            ",PROGRAMACION,SOURCE,DEPARTAMENTO,ACCESO,NUMERO_CR,IDLLAMADA,SUBZONA_ID,PROCESO ".
...
//---------------------insert pedido reagendamiento--------------------agendamiento-------------
    ...
    private function insertPedidoReagendamiento(){
        ...
        }else if($novedad=='YA ESTA ANULADO'||$novedad=='YA ESTA CUMPLIDO' ||$novedad=='YA ESTA AGENDADO'||$novedad=='YA ESTA ANULADO-CERRADO'|| $novedad=='ANULADO 42'||$novedad=='MIGRACION HFC' ||$novedad=='CAMBIO_NUMERO/PLAN' ||$novedad=='ERROR SIEBEL 8.1' ||$novedad=='PEDIDO EN OTRO CONCEPTO' ||$novedad=='YA ESTA ANULADO-PENDIENTE'
            ||$novedad=='YA ESTA ANULADO-PENDIENTE' ||$novedad=='YA ESTA CUMPLIDO–CERRADO' ||$novedad=='YA ESTA CUMPLIDO–PENDIENTE'||$novedad=='PENDIENTE RECONFIGUAR PEDIDO' || $novedad=='ANULADO 42' ||$novedad=='ANULADO AXGAR' ||$novedad=='MIGRACION HFC' ||$novedad=='ANULADO APRCT' ||$novedad=='PENDIENTE OTRO CONCEPTO'
            || $novedad=='PENDIENTE EN OTRO CONCEPTO' || $novedad=='NO DESEA EL SERVICIO' || $novedad=='YA ESTA CUMPLIDO'){
...
//-----------------------insertactivacion----------------
    ...
    private function insertTransaccionsiebelactivacion(){
        ...
        $pedido = json_decode(file_get_contents("php://input"),true);
        //var_dump($pedido);
        $column_names = array('ORDER_SEQ_ID','PEDIDO','REFERENCE_NUMBER','ESTADO','FECHA_CREACION','TAREA_EXCEPCION','FECHA_EXCEPCION','PRODUCTO','IDSERVICIORAIZ','TRANSACCION','CODIGO_CIUDAD','STATUS','ASESOR','FECHA_GESTION','TIPIFICACION','FECHA_INICIO','FECHA_FIN','DURACION','OBSERVACION','NUMERO_CR');
        $pedido=$pedido['pedido'];
        $keys = array_keys($pedido);
        $today = date("Y-m-d H:i:s");
        $today2 = date("Y-m-d");
        ...
        if(!empty($pedido)){
            ...
            $query = "insert into gestor_historico_activacion (ORDER_SEQ_ID,PEDIDO,REFERENCE_NUMBER,ESTADO,FECHA_CREACION,TAREA_EXCEPCION,FECHA_EXCEPCION,PRODUCTO,IDSERVICIORAIZ,TRANSACCION,CODIGO_CIUDAD,STATUS,ASESOR,FECHA_GESTION,TIPIFICACION,FECHA_INICIO,FECHA_FIN,DURACION,OBSERVACION,NUMERO_CR) values ('$ORDER_SEQ_ID','$PEDIDO','$REFERENCE_NUMBER','$ESTADO','$FECHA_CREACION','$TAREA_EXCEPCION','$FECHA_EXCEPCION','$PRODUCTO','$IDSERVICIORAIZ','$TRANSACCION','$CODIGO_CIUDAD','$STATUS','$ASESOR','$today','$TIPIFICACION','$FECHA_INICIO','$FECHA_FIN','$DURACION','$OBSERVACION','$NUMERO_CR') ";
...
private function guardarGestionAsignaciones()
{
    ...
    if($programado){
        echo "si entro";
        $sqlupdate="update informe_petec_pendientesm set PROGRAMACION='$programacion', RADICADO_TEMPORAL='NO',ASESOR='' WHERE STATUS in ('PENDI_PETEC','MALO') and PEDIDO_ID='$pedido' ";
