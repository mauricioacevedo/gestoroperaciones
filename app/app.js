			...
			function (data) {
				...
				$scope.peds = data.data;
                        ...
                        if(dat==204){
                                ...
                                //document.getElementById("warning").innerHTML="";
								$scope.pedido1=$scope.peds[1][0].PEDIDO_ID;
                                $scope.pedidoinfo=$scope.peds[1][0].PEDIDO_ID;
								$scope.InfoPedido.FUENTE=$scope.peds[1][0].FUENTE;
								$scope.fechaprogramacion=$scope.peds[1][0].PROGRAMACION;
								$scope.info.CONCEPTO_ID=$scope.peds[1][0].CONCEPTO_ID;

                                if(($scope.peds[1][0].STATUS=="PENDI_PETEC" || $scope.peds[1][0].STATUS=="PENDI_RENUMS")&&$scope.peds[1][0].ASESOR!=""){
                                        $scope.busy=$scope.peds[1][0].ASESOR;
                                        $rootScope.errorDatos="El pedido "+$scope.pedido1+" esta ocupado por "+$scope.peds[1][0].ASESOR;
                                }
...
app.run(['$rootScope', 'firebase', 'services', function ($rootScope, $firebase, services) {
	...
	$rootScope.transaccionActivacion = [
		...
		{
			TRANSACCION: 'ELIMINAR COMUNES',
			VALOR: 'ELIMINAR COMUNES'
		},
		{
			TRANSACCION: 'NUEVO',
			VALOR: 'NUEVO'
		...
		{
			TRANSACCION: 'ACTUALIZAR COMUNES',
			VALOR: 'ACTUALIZAR COMUNES'
		},
		{
			TRANSACCION: 'TRASLADO',
			VALOR: 'TRASLADO'
		},
		{
			TRANSACCION: 'ACTUALIZAR',
